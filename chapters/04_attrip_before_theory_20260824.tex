...
\vskip -0.15in
\end{table}
\section{检测框原型记忆与GLIP闭环自训练扩展}

前述实验主要从文本属性和知识蒸馏角度改善零样本细胞核检测。本节进一步研究检测结果本身能否反向参与下一轮视觉语言推理，并刻意使用 GLIP 作为后续学生网络，以区别于前文以 YOLOX 或 Mask R-CNN 为学生的 SKD 设置。实验包括无参数更新的框特征模板迭代、带参数更新的 GLIP 闭环自训练，以及带实例分割头的任务扩展。三个实验均在独立实验目录中完成，原始数据划分和标注文件保持不变。
...
\begin{equation}
\boldsymbol p_c^{(t)}=\frac{\sum_{i\in\mathcal I_c}s_i\boldsymbol z_i}{\sum_{i\in\mathcal I_c}s_i}.
\end{equation}
为减少单张图像偶然误检的影响，使用动量系数 $\beta=0.9$ 更新跨图像原型记忆 $\boldsymbol m_c^{(t)}=\beta\boldsymbol m_c^{(t-1)}+(1-\beta)\boldsymbol p_c^{(t)}$。随后将归一化后的原型写入该类别短语末尾的保留 token 位置，并与已经映射到视觉语言对齐空间的文本特征按 $\alpha=0.5$ 混合，再执行下一轮检测。整个过程冻结 GLIP 的全部参数，不进行反向传播。

表~\ref{tab:attrip_box_token} 给出了单类别 CoNSeP 和四类别 CCRCC 上的结果。CoNSeP 中，两轮模板迭代将 mAP 从 0.073 提升到 0.090，AP50 从 0.193 提升到 0.244，AR 从 0.143 提升到 0.204，说明较可靠的初始框特征能够补充文本提示对局部核形态描述的不足。CCRCC 上，基础 GLIP 的四类区分能力本身较弱；将阈值降至 0.35 后结果与基线基本相同，而使用阈值 0.55 连续迭代两轮时 mAP 降至 0.001。该负结果表明，模板 token 并不保证单调改进：当初始高置信度框集中在少数类别时，原型记忆会放大类别偏置并形成确认偏差。
...
\subsection{GLIP作为学生网络的闭环自训练}

在无训练模板迭代的基础上，进一步引入教师--学生式参数更新。教师仍为带最优属性提示的 GLIP，并在 CoNSeP 的 108 张训练图像上执行两轮框特征模板推理；以 0.5 为伪标签阈值，共保留 786 个高置信度框，覆盖全部训练图像。学生网络不再使用 YOLOX，而是采用与教师相同的 GLIP 结构并从原始预训练权重初始化，在伪标签上训练 4 个 epoch。测试阶段分别关闭和开启框特征模板，以分离参数更新和原型记忆的贡献。该设置严格使用训练划分生成伪标签，并在独立验证划分上评价，不读取验证标注参与训练。
...
表~\ref{tab:attrip_glip_student} 表明，仅使用伪标签更新 GLIP 参数时，mAP 由 0.073 提升至 0.089，但仍略低于无训练模板迭代的 0.090；将两种机制组合后，mAP、AP50 和 AP75 分别达到 0.096、0.257 和 0.049，为本组精度指标的最高值。这说明伪标签参数更新能够吸收训练集中的稳定目标结构，而框特征模板能够在测试时继续补充当前图像的局部外观。另一方面，组合方法的 AR 为 0.183，低于无训练模板迭代的 0.204。原因在于 786 个高置信度伪框相对于训练集真实实例数量较为稀疏，学生更倾向于学习高精度、低召回的决策边界。因此，本实验支持“两个闭环在精度上互补”，但不支持“每轮迭代对所有指标均单调改善”的更强结论。

\subsection{带实例分割头的任务扩展}

为验证本章方法并非只能输出检测框，在 GLIP 主干后连接轻量级 tiny mask 头，并在 MoNuSeg 测试划分得到的 224 个图像块上进行实例级评价。该分支由候选框对应的多尺度视觉特征预测实例掩膜，检测与分割采用同一主干和类别语义。表~\ref{tab:attrip_mask_head} 为隔离复测结果。bbox AP 和 AP50 分别达到 0.382 和 0.830，说明增加掩膜分支后仍保留较强的细胞核定位能力；segm AP、AP50 和 AP75 分别为 0.423、0.804 和 0.407，表明轻量级掩膜头能够将框级语义进一步转化为像素级实例边界。由于 bbox AP 与 segm AP 的侧重点不同，二者不作数值上的直接优劣比较，而分别用于评价定位和分割质量。

\begin{table}[htbp]
\centering
\caption{带实例分割头的检测与分割结果}
...
\subsection{本章方法相对NucleiPrompter的推进}
