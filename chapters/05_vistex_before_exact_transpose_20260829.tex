...
\begin{figure}[!htbp]
  ...
  \begin{minipage}[c]{0.49\textwidth}
    \centering
    \includegraphics[width=\linewidth,viewport=0 1800 1840 3750,clip]{chapters/PAPERS/VisTex_ICCV_ver/ODinW13_sd.png}
  \end{minipage}\hfill
  \begin{minipage}[c]{0.49\textwidth}
    \centering
    \includegraphics[width=\linewidth,viewport=0 0 1840 1800,clip]{chapters/PAPERS/VisTex_ICCV_ver/ODinW13_sd.png}
  \end{minipage}
  \caption{ODinW13 开放集迁移结果}
  \label{fig:vistex_odinw}
\end{figure}
图~\ref{fig:vistex_odinw} 将原先纵向排列的 13 个数据集拆分为左右两个横向区域，在不改变任何预测框和方法顺序的前提下压缩纵向版面。左侧依次展示 Aquarium、AerialDrone、Rabbits、EgoHands、Mushrooms、PascalVOC 与 Packages，右侧展示 Pistols、Pothole、Raccoon、Shellfish、Thermal 与 Vehicles；每个数据集内部六列依次对应 Meta-DETR、DiGeo、DeFRCN、MFD、MQ-Det 和 VisTex-GLIP。

从目标属性看，Aquarium、AerialDrone 与 Pothole 等场景同时包含小目标、俯视视角或低对比度背景，零样本模型容易把局部纹理相近区域当作目标；Rabbits、Raccoon 和 Vehicles 的目标轮廓较完整，视觉提示更容易形成稳定的形状原型。VisTex-GLIP 在大多数行中表现为绿色真阳性框增加、黄色漏检框减少，说明支持图像提供的颜色、局部纹理和典型轮廓可以弥补类别名称描述不足。对 Packages、Thermal 等域偏移更强的任务，提升并不完全一致，表明视觉 token 的有效性仍受支持样本代表性与成像模态差异制约。该规律与定量结果一致：方法的优势不是把所有任务统一抬高相同幅度，而是在文本语义不足时利用少量实例外观恢复目标--文本对齐。
